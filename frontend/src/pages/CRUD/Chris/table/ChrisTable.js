...
import * as dataFormat from 'pages/CRUD/Chris/table/ChrisDataFormatters';

import actions from 'actions/chris/chrisListActions';
...
const ChrisTable = () => {
  ...
  const count = useSelector((store) => store.chris.list.count);
  const modalOpen = useSelector((store) => store.chris.list.modalOpen);
  const rows = useSelector((store) => store.chris.list.rows);
  const idToDelete = useSelector((store) => store.chris.list.idToDelete);
  ...
  return (
    <div>
      <Widget title={<h4>Chris</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/chris/new'>
        ...
        >
          <DataGrid
            ...
            loading={loading}
            onRowClick={(e) => {
              history.push(`/admin/chris/${e.id}/edit`);
            }}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Chris'
                : 'http://localhost:8080/api-docs/#/Chris'
            }
          >
            API documentation for chris
          </LinkMaterial>
        </div>
      </Widget>

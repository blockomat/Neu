...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/chris/autocomplete`, { params });
  return response.data;
}

const ChrisSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.chris.hasPermissionToCreate,
});

export default connect(select)(ChrisSelectItem);
